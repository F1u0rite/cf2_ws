...
class NiceGuiNode(Node):
    ...
    @staticmethod
    def _default_ring_config_paths() -> list[Path]:
        paths = []
        env_path = Path(str(Path.home() / 'Documents' / 'cf2_ws' / 'src' / 'ring_flight' / 'config' / 'rings.yaml'))
        paths.append(env_path)
        try:
            share_path = Path(get_package_share_directory('ring_flight')) / 'config' / 'rings.yaml'
    ...
    def _load_rings(self):
        for path in self._default_ring_config_paths():
            if path.exists():
                try:
                    with path.open('r', encoding='utf-8') as f:
                        data = yaml.safe_load(f) or {}
                    rings = data.get('rings', [])
                    self.get_logger().info(f'Loaded ring config for GUI: {path}')
                    return rings
                except Exception as exc:
                    self.get_logger().warning(f'Failed to load ring config {path}: {exc}')
        self.get_logger().info('No ring config found for GUI visualization.')
        return []

    def _add_ring_visuals(self, scene):
        rings = self._load_rings()
        for ring in rings:
            center = ring.get('position', [0.0, 0.0, 0.0])
            orientation = ring.get('orientation', [0.0, 0.0, 0.0])
            radius = float(ring.get('radius', 0.3))
            yaw = float(orientation[2]) if len(orientation) >= 3 else 0.0

            # Draw a hollow hoop as sampled points in the ring plane.
            tangent = [-math.sin(yaw), math.cos(yaw), 0.0]
            up = [0.0, 0.0, 1.0]
            samples = 36
            bead_r = 0.012
            for i in range(samples):
                theta = 2.0 * math.pi * i / samples
                px = center[0] + radius * (math.cos(theta) * tangent[0] + math.sin(theta) * up[0])
                py = center[1] + radius * (math.cos(theta) * tangent[1] + math.sin(theta) * up[1])
                pz = center[2] + radius * (math.cos(theta) * tangent[2] + math.sin(theta) * up[2])
                scene.sphere(bead_r).move(px, py, pz).material('#3388ff')

            scene.sphere(0.025).move(center[0], center[1], center[2]).material('#ffaa00')
...
# Work around environments where WebSocket upgrades are blocked by proxy/filter.
app.config.socket_io_js_transports = ['polling']
ui_run.APP_IMPORT_STRING = f'{__name__}:app'  # ROS2 uses a non-standard module name, so we need to specify it here
ui.run(uvicorn_reload_dirs=str(Path(__file__).parent.resolve()), favicon='🤖', port=8081)
